...
const validUsername = 'okdjmusical';
const validPassword = '123456q';
...
// Endpoint untuk hapus device
// Jangan lupa pastikan ini sudah ada di atas (sekali saja!)
app.use(express.json());

// Revisi /removeDevice endpoint
app.post('/removeDevice', (req, res) => {
  const { hostName } = req.body; // ✅ Langsung ambil dari JSON body
  console.log('Received removeDevice request for:', hostName);

  // Cek validitas hostName
  if (!hostName) {
    console.log('No hostName in request');
    return res.status(400).send('Bad Request: Missing hostName.');
  }

  // Cek apakah device ada di registeredDevices
  if (registeredDevices.has(hostName)) {
    registeredDevices.delete(hostName);
    lastHeartbeatTime.delete(hostName);

    // Save to file biar persistent
    saveRegisteredDevicesToFile();

    console.log(`✅ Device ${hostName} removed successfully.`);
    res.status(200).send(`Device "${hostName}" removed successfully.`);
  } else {
    console.log(`❌ Device not found: ${hostName}`);
    res.status(404).send('Device not found.');
  }
});
app.get('/getSpiffsFile', (req, res) => {
  const filePath = path.join(__dirname, req.query.path); // Cek apakah sesuai
  console.log("📦 Path request:", req.query.path);
...
app.post('/clearDataFolder', (req, res) => {
  const dataDir = path.join(__dirname, 'data');
  if (fs.existsSync(dataDir)) {
    fs.rmSync(dataDir, { recursive: true, force: true });
    console.log("🧹 Semua file lama di folder data dihapus!");
  }
  fs.mkdirSync(dataDir, { recursive: true });
  res.status(200).send('Folder data dikosongkan!');
});
...
app.post('/uploadFile', (req, res) => {
  ...
  // ⚠️ JANGAN HAPUS SEMUA FILE DI SINI!
  // fs.rmSync(dataDir, { recursive: true, force: true });  // 🚫 HAPUS INI

  fs.mkdirSync(dir, { recursive: true });  // Pastikan folder ada
  ...
  req.on('end', () => {
    writeStream.end();
    console.log("✅ Upload selesai & file disimpan:", filePath);
...
app.get('/listSpiffsFiles', (req, res) => {
  const basePath = path.join(__dirname, 'data'); // cuma folder data
  let fileList = [];
  ...
  function walk(dir, base) {
    ...
    files.forEach(file => {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        // Tambahkan prefix "data/" biar ESP32 tau
        fileList.push("data/" + relPath.replace(/\\/g, '/'));
      }
    });
  }
...
app.post('/upload', upload.single('firmwareFile'), (req, res) => {
	...
	// Tambahin pengecekan & hapus file target kalau sudah ada
	if (fs.existsSync(filePath)) {
		try {
			fs.unlinkSync(filePath); // Hapus dulu biar rename lancar
		} catch (err) {
			console.error('Error deleting existing firmware file:', err.message);
			res.status(500).send('Failed to delete existing firmware file.');
			return;
		}
	}

	try {
		fs.renameSync(req.file.path, filePath); // Rename final
		...
		console.error('Error renaming firmware file:', err.message);
		res.status(500).send('Failed to rename firmware file.');
	}
});
